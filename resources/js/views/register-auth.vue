  ...
  <div>
    ...
    <div v-if="loaded">
      <navbar class="d-flex justify-content-center">
        <router-link :to="{name: 'register'}" class="auth-action fs-3 px-5 active-link"
          >Register</router-link
        >
        <router-link :to="{name: 'login'}" class="auth-action fs-3 px-5"
          >Login</router-link
        >
      </navbar>
      <div class="d-flex justify-content-center">
        <AppForm v-on:form-submit="authenticate">
          ...
          <button class="auth-btn py-2 mb-5" type="submit">Register</button>
        </AppForm>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    authenticate: async function (formObj, authFormComponent) {
      ...
      this.makeRequest(this.httpMethods.post, "/api/users", {
        payload: payload,
        handle201: {
